...
<center>
	<?php 
	$qq="SELECT *,CONCAT(first_name,' ',last_name) AS name FROM feedback INNER JOIN `patients` USING(patient_id)";
	$rs=select($qq);
	if(sizeof($rs)>0){ ?>
	...
	<form>
		<table class="table">
			<tr>
				...
				<th style="color: black">feedback</th>
		...
		foreach ($rs as $r) { ?>
			<tr>
				...
				<td style="color: black"><?php echo $r['feedback'] ?></td>
