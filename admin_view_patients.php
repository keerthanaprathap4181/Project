...
$qq="SELECT *,concat(first_name,' ',last_name) as name FROM patients inner join login using(login_id) where status='1'";

if (isset($_POST['search'])) 
{
	extract($_POST);
	$qq="SELECT *,CONCAT(first_name,' ',last_name) AS name FROM `patients` INNER JOIN login USING (login_id) WHERE `status`=1 AND first_name LIKE '%$item%'";
}
...
<div class="inner-block">
	<form method="post">
		<table class="table" style="width: 400px">
			<tr>
				<th><input type="text" name="item" class="form-control" placeholder="Search"></th>
