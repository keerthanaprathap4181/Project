...
<table class="table" style="width: 700px">
 ...
 <?php 
    $q1="SELECT * FROM `schedule` INNER JOIN `bookings` USING(schedule_id) where patient_id='$patientid'";
    $r=select($q1);
    foreach ($r as $key) 
    {
     ?>
    <tr>
      ...
      <td style="color: black"><?php echo $key['start_time'] ?> - <?php echo $key['end_time'] ?></td>
      <td style="color: black"><?php echo $key['fee_amount'] ?></td>
      <?php 
        if ($key['status']=="Accepted") 
        {
          ?>
        <td><a class="btn btn-primary" href="patientmakepayment.php?id=<?php echo $key['book_id'] ?>&amt=<?php echo $key['fee_amount'] ?>">Make Payment</a></td>
          <?php
        }else{
          ?>
